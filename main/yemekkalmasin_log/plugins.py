...
class LatestRequests(Plugin):
    def template_context(self):
        return {'requests': RequestLog.objects.all()[:5]}


class TrafficInformation(Plugin):
    def template_context(self):
        INFO_TABLE = ('today', 'this_week', 'this_month', 'this_year', 'all')
        INFO_TABLE_QUERIES = [getattr(RequestLog.objects, query, None)() for query in INFO_TABLE]
